...
@Component
public class SpecificationMangaMapper {

    public static Specification<MangaEntity> FromSearchFilterToSpecification(List<SearchFilter> searchFilters) {
        Specification spec = Specification.where(com.example.myanimelibrary.infrastructure.mapper.SpecificationMapper.createSpecification(searchFilters.remove(0)));
        for (SearchFilter searchFilter : searchFilters) {
            spec = spec.and(SpecificationMangaMapper.createSpecification(searchFilter));
        }
        return spec;
    }

    public static Specification<MangaEntity> createSpecification(SearchFilter input) {
